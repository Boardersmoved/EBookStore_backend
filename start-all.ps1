...
# 检查 Docker
Write-Host "[1/7] 检查 Docker 服务..." -ForegroundColor Yellow
try {
    docker ps | Out-Null
...
# 启动 Redis
Write-Host "[2/7] 启动 Redis 容器..." -ForegroundColor Yellow
docker start ebook
Start-Sleep -Seconds 3
Write-Host "✓ Redis 已启动" -ForegroundColor Green

# 启动 Eureka Server
Write-Host "[3/7] 启动 Eureka Server (端口 8761)..." -ForegroundColor Yellow
...
# 启动 Author Service
Write-Host "[4/7] 启动 Author Service (端口 8081)..." -ForegroundColor Yellow
...
# 启动 Price Calculator Service (函数式服务)
Write-Host "[5/7] 启动 Price Calculator Service (端口 8084)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$rootDir\price-calculator-service'; ..\mvnw.cmd spring-boot:run" -WindowStyle Normal
Start-Sleep -Seconds 10
Write-Host "✓ Price Calculator Service 启动中..." -ForegroundColor Green

# 启动主应用
Write-Host "[6/7] 启动 EBookStore Backend (端口 8082)..." -ForegroundColor Yellow
...
# 启动 API Gateway
Write-Host "[7/7] 启动 API Gateway (端口 8080)..." -ForegroundColor Yellow
...
Write-Host "  - 价格计算服务: http://localhost:8084/api/price/health" -ForegroundColor White
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
